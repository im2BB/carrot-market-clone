    ...
    <div className="p-7 max-w-6xl mx-auto">
      ...
      {events.length === 0 ? (
        <div className="text-center py-12">
          <p className="text-neutral-400 text-lg mb-4">등록된 이벤트가 없습니다.</p>
          <Link
            href="/add-event"
            className="bg-orange-500 hover:bg-orange-600 text-white px-6 py-3 rounded-lg font-medium transition-colors"
          >
                ...
                <div className="flex items-start gap-6">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-neutral-400 mb-4">
                      <div>
                        <span className="text-orange-400 font-medium">기간:</span> {format(new Date(event.start_date), "yyyy년 MM월 dd일 HH:mm", { locale: ko })} ~ {format(new Date(event.end_date), "yyyy년 MM월 dd일 HH:mm", { locale: ko })}
                      </div>
                      <div>
                        <span className="text-orange-400 font-medium">등록일:</span> {format(new Date(event.created_at), "yyyy년 MM월 dd일", { locale: ko })}
                      </div>
                    </div>
                    {event.link && (
                      <div className="text-sm text-neutral-400 mb-4">
                        <span className="text-orange-400 font-medium">링크:</span> 
                        <a href={event.link} target="_blank" rel="noopener noreferrer" className="text-blue-400 hover:text-blue-300 underline ml-2">
                          {event.link}
                        </a>
                      </div>
                    )}
