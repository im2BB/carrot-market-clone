...
export default async function SearchPage({ searchParams }: { searchParams: { query?: string } }) {
  ...
  if (!query) {
    return (
      <div className="p-10 text-center text-neutral-400">검색어를 입력해주세요.</div>
    );
  }
  ...
  return (
    ...
    <div className="p-7">
     <SearchBar />
      ...
      {products.length === 0 ? (
        <div className="flex flex-col items-center justify-center text-xl text-neutral-400 text-center py-20">검색 결과가 없습니다.</div>
      ) : (
          ...
          {products.map((product) => {
            ...
            return (
              ...
              <div className="flex justify-center items-center">
                <h1 className="text-sm mb-6 text-center">검색 결과: "{query}"</h1>
              </div>
              <Link
