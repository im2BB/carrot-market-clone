  ...
  const interceptAction = async (_: unknown, formData: FormData) => {
    try {
      ...
      if (!uploadResponse.ok) {
        ...
        return {
          fieldErrors: { image: ["이미지 업로드에 실패했습니다."] },
          formErrors: [],
        };
      }
      ...
      <form action={handleSubmit} className="space-y-5">
        <div>
          ...
          {state &&
            ...
            (state as any).fieldErrors?.image && (
              <div className="text-red-500 text-sm mt-1">
                {(state as any).fieldErrors.image.join(", ")}
              </div>
            )}
        </div>
        ...
        <Input
          ...
          errors={
            state && "fieldErrors" in state
              ? (state as any).fieldErrors?.title
              : undefined
          }
        />
        ...
        <div>
          ...
          {state &&
            "fieldErrors" in state &&
            (state as any).fieldErrors?.description && (
              <div className="text-red-500 text-sm mt-1">
                {(state as any).fieldErrors.description}
              </div>
            )}
        ...
        <div className="grid grid-cols-2 gap-4">
          <Input
            ...
            errors={
              state && "fieldErrors" in state
                ? (state as any).fieldErrors?.start_date
                : undefined
            }
          />
          <Input
            ...
            errors={
              state && "fieldErrors" in state
                ? (state as any).fieldErrors?.end_date
                : undefined
            }
          />
        </div>
        ...
        {state &&
          "fieldErrors" in state &&
          (state as any).fieldErrors?._form && (
            <div className="text-red-500 text-sm">
              {(state as any).fieldErrors._form.join(", ")}
            </div>
          )}

        <Button text={(state as any)?.pending ? "등록 중..." : "등록하기"} />
      </form>
    </div>
  );
}
